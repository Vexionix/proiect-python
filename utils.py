...
def parse_wikipedia_number_string_to_int(number_string):
    ...
    process. More specifically used for area
    :param number_string: A string containing a numerical value
    (specifically country's area)
...
def parse_capital_text(capital):
    """
    Function use to parse capital text based on the way wikipedia formats
    strings Different formats found are: displaying dates since when the
    capital has been instated or if its temporary or de facto or referencing
    other pages or empty parenthesis (ex Suedia) etc.
    :param capital: Capital text string from wikipedia's country page infobox
    :return: parsed capital text string, only containing
    ...
    # Filter out useless parentheses (such as the date since
    # when a capital has become one)
    result = re.sub(
        r'\([^)]*?(din|note).*?\)', '', result, flags=re.IGNORECASE)
    # Filter out placeholders (Bolivia {{PAGENAME}})
    result = re.sub(r'\{\{.*?\}\}', '', result)
    # Add a space between closing parenthesis and
    # the following letter if it's the case
...
def parse_neighbors_text(neighbors):
    """
    Function use to parse neighbors text based on the way wikipedia
    formats strings and removing data that is not deemed necessary
    :param neighbors: the text containing the list of neighbors of
    the country extracted from the wikipedia page
...
def parse_languages_text(languages):
    """
    Function use to parse languages text based on the way wikipedia
    formats strings and removing data that is not deemed necessary
    :param languages: the text containing the list of languages spoken
    in the country extracted from the wikipedia page
    :return: the parsed value removing useless characters - the clean string
    """
...
def parse_timezone(timezone):
    ...
    :param timezone: the time zone of the country extracted from
    the wikipedia page
    :return: the parsed value removing useless characters - the clean string
    """
