...
def scrape_wikipedia():
    ...
    # Link to the table containing the toop of countries sorted by population from where
    # The name of the country, the population as well as the link to each country's page
    # can be extracted to get more details for each
    ...
    # Parse the received content in a more manageable way using BeautifulSoup
    ...
    for row in rows[1:]:
        # Get each cell of the current row containing data such as "Name", "Population"
        cols = row.find_all('td')
        if len(cols) > 2:
            # name_link will contain the link to the country if successful, for example it extracts
            # the <a href="...">China</a> field for China
            name_link = cols[1].find('a')
            # Extra check in case of having a link to an image prior to the country name as it
            # is an inconsistent thing, some countries do have this, others don't
            if name_link and '.svg' in name_link['href']:
                name_link = name_link.findNext('a')
            # If nothing was found to fit our search criterion we skip to the next row
            if not name_link:
                continue

            # If the previous step was successful, extract the country name from the name_link field
            ...
            # Check if the country is already added in the database before proceeding with scraping
            cursor.execute('''SELECT COUNT(*) FROM countries WHERE name = ?''', (name,))
            existing_entry = cursor.fetchone()[0]
            if existing_entry == 0:

                # As the data is saved in string format, extract as a string first and then
                # convert to integer if possible, but the data is consistent here
                population_as_string = cols[2].text.strip().replace(".", "")
                population = parse_population_string_to_int(population_as_string)

                # Get all data from the current page and store it in its respective variable
                area, density, capital, neighbors, languages, timezone, regime = scrape_country_details(country_url)

                # If no density was found calculate it manually with a formula that provides an approximate result
                if density == -1.0:
                    if area != -1:
...
def scrape_country_details(url):
    ...
    if infobox:
        ...
        for row in rows:
            # Use both th and td to get matching search criteria, inline elements that have both
            # a subtitle next to them and descriptive text next to them (example: "Suprafata    494.595km^2")
            # Select each of the searched element based on matching title of the searched criterion, if the
            # title is "densitate" then "density" should take the value found in the td element proceeding the
            # header (th)
            th = row.find('th')
            td = row.find('td')
            ...
            if th and td:
                ...
                # For each of the searched field try to match the expected title to the current row
                # one and take the value describing the row and save it in the respective variable
                ...
                # Save the previous row title (specifically for area, because the data is displayed as
                # "Suprafata
                #  - Total(a)          (value)"
                # But looking for "total" is not enough because there are multiple rows that can fit this
                # criterion and as such lead to errors
                old_row_title = key

    # Return the found values or the default ones for the fields that did not have a corresponding
